 ...
 * tema 0 y 2 para los temas 1-2, y ese número **no crece**: una tabla nueva, una columna nueva o una
 * ruta nueva entran en el recorrido de una prueba que ya existe. Si alguien añade un método de test
...
it('el módulo generado trae las piezas del contrato, con su techo de pruebas', function (
    ...
    ], 'R32: el grupo de la subfuncionalidad son 6 piezas fijas; estas son las de los temas 0 y 1-2.');
...
it('ninguna pieza generada es un placebo', function () {
    ...
    foreach (['InvoiceScaffoldTest', 'InvoiceSchemaTest'] as $pieza) {
...
it('las piezas cuelgan de la base del grupo, no del TestCase del proyecto', function (ModuleMode $modo, ?string $contexto, string $carpeta, string $prefijo) {
    ...
    foreach (['ScaffoldTest', 'SchemaTest'] as $pieza) {
