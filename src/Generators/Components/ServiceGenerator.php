...
class ServiceGenerator extends AbstractComponentGenerator
{
     ...
     *
     * Orquesta la creación de la estructura de directorios y la generación de los archivos.
     *
     * @return void
     */
    public function generate(): void
    {
        $servicesDir = $this->getComponentBasePath(self::SERVICES_DIR);
        $contractsDir = $this->getComponentBasePath(self::CONTRACTS_DIR);
        $this->ensureDirectoryExists($servicesDir);
        $this->ensureDirectoryExists($contractsDir);

        $this->generateServiceContract($contractsDir);
        $this->generateServiceImplementation($servicesDir);
    }

    /**
     * Genera la interfaz del servicio.
     *
     * @param string $contractsDir El directorio donde se guardarán los contratos.
     * @return void
     */
    protected function generateServiceContract(string $contractsDir): void
    {
        ...
        $this->putFile(
            "{$contractsDir}/{$serviceInterfaceName}.php",
            $stubContent,
     ...
     *
     * @param string $servicesDir El directorio donde se guardarán las implementaciones.
     * @return void
     */
    protected function generateServiceImplementation(string $servicesDir): void
    {
        $serviceInterfaceName = $this->serviceName . self::INTERFACE_SUFFIX;
        ...
        $this->putFile(
            "{$servicesDir}/{$this->serviceName}.php",
            $stubContent,
